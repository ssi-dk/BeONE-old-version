...
def update_dropdowns(DB, selected_run):
    if DB is None:
        DB = 'bifrost_upgrade_test'
    return hc.dropdowns_options(DB, selected_run)
...
# )
# def update_selected_samples(ignore, rows, derived_virtual_selected_rows):
...
def render_content(tab):
    ...
    if tab == 'projects-tab':
        ...
        column_names =[{"name": i, "id": i} for i in columns]
        ...
        return hc.html_tab_projects(samples, column_names)
