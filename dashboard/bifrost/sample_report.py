...
def sample_report(data):
    ...
    return [
        ...
        dbc.Row(
            ...
            ], justify="between", style={'padding-top':'10px'}
        ),
...
def check_test(test_name, sample):
    test_path = "properties.ssi_stamper.summary." + test_name + ".status"
            ...
            [
                ...
                dbc.CardBody([
                    html.Div([
                        ...
                        admin.sample_radio_feedback(sample, n_sample)], className="pretty_container ten columns")
                ])
...
def html_test_tables(sample_data):
    qc_to_check = ["ssi_stamper"]
    rows = []
    for key, value in sample_data.items():
        if key.startswith("properties.ssi_stamper.summary.whats_my_species") \
                or key.startswith("properties.ssi_stamper.summary.assemblatron"):
            if pd.isnull(value):
                value = "nan"
            name_v = key.split(":")
            values = value.split(':')
...
def html_sample_tables(sample_data, **kwargs):
    ...
    if get(sample_data, "properties.sample_sheet.sample_name"):
        ...
        sample_sheet_table = html_table([
            ...
            {
                "list": ["Read file",
                         str(get(sample_data, "reads", [""])[0]).split("/")[-1]],
