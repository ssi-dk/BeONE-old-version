...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Store(id='db-store', data = [], storage_type='session'),
...
@app.callback(
    ...
    [Input('db-radiobuttons', 'value')]
)
def select_db(db):
    print('select_db')

    if db == 'remote':
        uri = 'mongodb://139.59.159.69:27017/EOSP_public'
        connection = bifrostapi.add_URI(mongoURI=uri, connection_name='remote')
    else:
        uri = 'mongodb://localhost:27017/EOSP_private'
        connection = bifrostapi.add_URI(mongoURI=uri, connection_name='local')
